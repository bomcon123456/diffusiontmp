...
from dataset import NVIDIAImageFolderDataset
...
def calculate_inception_stats(
    ...
    dataset_obj = NVIDIAImageFolderDataset(path=image_path, max_size=num_expected, random_seed=seed)
    ...
    for images in tqdm.tqdm(data_loader, unit='batch', disable=(dist.get_rank() != 0)):
        torch.distributed.barrier()
        if images.shape[0] == 0:
            continue
        if images.shape[1] == 1:
